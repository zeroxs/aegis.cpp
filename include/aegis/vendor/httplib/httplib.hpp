...
public:
  ...
  server & del(const char * pattern, HandlerWithContentReader handler);
...
inline result ClientImpl::del(const char * path)
{
  return del(path, Headers(), std::string(), nullptr);
}

inline result ClientImpl::del(const char * path, const Headers & headers)
{
  return del(path, headers, std::string(), nullptr);
}

inline result ClientImpl::del(const char * path, const char * body,
  size_t content_length,
  const char * content_type)
{
...
inline result ClientImpl::del(const char * path, const std::string & body,
  const char * content_type)
{
