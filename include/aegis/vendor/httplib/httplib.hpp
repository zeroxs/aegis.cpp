...
struct response;
using response_handler = std::function<bool(const response &response)>;
...
using multipart_form_data_items = std::vector<multipart_form_data>;
using multipart_form_data_map = std::multimap<std::string, multipart_form_data>;

class data_sink {
public:
  data_sink() : os(&sb_), sb_(*this) {}

  data_sink(const data_sink &) = delete;
  data_sink &operator=(const data_sink &) = delete;
  data_sink(data_sink &&) = delete;
  data_sink &operator=(data_sink &&) = delete;
...
private:
  ...
  public:
    explicit data_sink_streambuf(data_sink &sink) : sink_(sink) {}
...
using content_provider =
    std::function<bool(size_t offset, size_t length, data_sink &sink)>;

using content_provider_without_length =
    std::function<bool(size_t offset, data_sink &sink)>;

using content_receiver_with_progress =
    std::function<bool(const char *data, size_t data_length, uint64_t offset,
                       uint64_t total_length)>;

using content_receiver =
    std::function<bool(const char *data, size_t data_length)>;

using multipart_content_header =
    std::function<bool(const multipart_form_data &file)>;

class content_reader {
public:
  using reader = std::function<bool(content_receiver receiver)>;
  using multipart_reader = std::function<bool(multipart_content_header header,
                                             content_receiver receiver)>;

  content_reader(reader reader, multipart_reader multipart_reader)
      : reader_(std::move(reader)),
        multipart_reader_(std::move(multipart_reader)) {}

  bool operator()(multipart_content_header header,
                  content_receiver receiver) const {
    return multipart_reader_(std::move(header), std::move(receiver));
  }

  bool operator()(content_receiver receiver) const {
    return reader_(std::move(receiver));
  }

  reader reader_;
  multipart_reader multipart_reader_;
};
...
struct request {
  ...
  std::string target;
  params params;
  multipart_form_data_map files;
  ranges ranges;
  match matches;

  // for client
  response_handler response_handler;
  content_receiver_with_progress content_receiver;
  progress progress;
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
  const SSL *ssl;
#endif
  ...
  multipart_form_data get_file_value(const char *key) const;
...
struct response {
  ...
  void set_content_provider(
      size_t length, const char *content_type, content_provider provider,
      const std::function<void()> &resource_releaser = nullptr);

  void set_content_provider(
      const char *content_type, content_provider_without_length provider,
      const std::function<void()> &resource_releaser = nullptr);
...
class thread_pool : public task_queue {
public:
  explicit thread_pool(size_t n) : shutdown_(false) {
    while (n) {
...
private:
  struct worker {
    explicit worker(thread_pool &pool) : pool_(pool) {}
...
using socket_options = std::function<void(socket_t sock)>;
...
public:
  ...
  enum class handler_response {
    Handled,
    Unhandled,
  };
  using handler_with_response =
      std::function<handler_response(const request &, response &)>;

  using handler_with_content_reader = std::function<void(
      const request &, response &, const content_reader &content_reader)>;

  using expect_100_continue_handler =
  ...
  server &post(const char *pattern, handler_with_content_reader handler);
  server &post(const char *pattern, size_t pattern_len,
               handler_with_content_reader handler);
  server &put(const char *pattern, handler handler);
  server &put(const char *pattern, size_t pattern_len, handler handler);
  server &put(const char *pattern, handler_with_content_reader handler);
  ...
  server &patch(const char *pattern, handler_with_content_reader handler);
  server &patch(const char *pattern, size_t pattern_len,
                handler_with_content_reader handler);
  server &del(const char *pattern, handler handler);
  server &del(const char *pattern, size_t pattern_len, handler handler);
  server &del(const char *pattern, handler_with_content_reader handler);
  server &del(const char *pattern, size_t pattern_len,
                 handler_with_content_reader handler);
  ...
  server &set_error_handler(handler_with_response handler);
  server &set_error_handler(handler handler);
  server &set_exception_handler(exception_handler handler);
  server &set_pre_routing_handler(handler_with_response handler);
  server &set_post_routing_handler(handler handler);

  server &set_expect_100_continue_handler(expect_100_continue_handler handler);
  server &set_logger(logger logger);

  server &set_tcp_nodelay(bool on);
  server &set_socket_options(socket_options socket_options);
  ...
  std::function<task_queue *(void)> new_task_queue;
...
private:
  ...
  using handlers_for_content_reader =
      std::vector<std::pair<std::regex, handler_with_content_reader>>;

  socket_t create_server_socket(const char *host, int port, int socket_flags,
                                socket_options socket_options) const;
  ...
  dispatch_request_for_content_reader(request &req, response &res,
                                      ...
                                      const handlers_for_content_reader &handlers);
  ...
  read_content_with_content_receiver(stream &strm, request &req, response &res,
                                     content_receiver receiver,
                                     multipart_content_header multipart_header,
                                     content_receiver multipart_receiver);
  bool read_content_core(stream &strm, request &req, response &res,
                         content_receiver receiver,
                         multipart_content_header mulitpart_header,
                         content_receiver multipart_receiver);
  ...
  std::vector<mount_point_entry> base_dirs_;
  ...
  handlers_for_content_reader post_handlers_for_content_reader_;
  handlers put_handlers_;
  handlers_for_content_reader put_handlers_for_content_reader_;
  handlers patch_handlers_;
  handlers_for_content_reader patch_handlers_for_content_reader_;
  handlers delete_handlers_;
  handlers_for_content_reader delete_handlers_for_content_reader_;
  handlers options_handlers_;
  handler_with_response error_handler_;
  exception_handler exception_handler_;
  handler_with_response pre_routing_handler_;
  handler post_routing_handler_;
  logger logger_;
  expect_100_continue_handler expect_100_continue_handler_;

  bool tcp_nodelay_ = CPPHTTPLIB_TCP_NODELAY;
  socket_options socket_options_ = default_socket_options;
};
...
class client_impl {
public:
  explicit client_impl(const std::string &host);

  explicit client_impl(const std::string &host, int port);
  ...
  result get(const char *path, content_receiver content_receiver);
  result get(const char *path, const headers &headers,
             content_receiver content_receiver);
  result get(const char *path, content_receiver content_receiver,
             progress progress);
  result get(const char *path, const headers &headers,
             content_receiver content_receiver, progress progress);
  result get(const char *path, response_handler response_handler,
             content_receiver content_receiver);
  result get(const char *path, const headers &headers,
             response_handler response_handler,
             content_receiver content_receiver);
  result get(const char *path, response_handler response_handler,
             content_receiver content_receiver, progress progress);
  result get(const char *path, const headers &headers,
             response_handler response_handler, content_receiver content_receiver,
             progress progress);

  result get(const char *path, const params &params, const headers &headers,
             progress progress = nullptr);
  result get(const char *path, const params &params, const headers &headers,
             content_receiver content_receiver, progress progress = nullptr);
  result get(const char *path, const params &params, const headers &headers,
             response_handler response_handler, content_receiver content_receiver,
             progress progress = nullptr);
  ...
  result post(const char *path, size_t content_length,
              content_provider content_provider, const char *content_type);
  result post(const char *path, content_provider_without_length content_provider,
              const char *content_type);
  result post(const char *path, const headers &headers, size_t content_length,
              content_provider content_provider, const char *content_type);
  result post(const char *path, const headers &headers,
              content_provider_without_length content_provider,
              const char *content_type);
  result post(const char *path, const params &params);
  result post(const char *path, const headers &headers, const params &params);
  result post(const char *path, const multipart_form_data_items &items);
  result post(const char *path, const headers &headers,
              const multipart_form_data_items &items);
  result post(const char *path, const headers &headers,
              const multipart_form_data_items &items, const std::string &boundary);
  ...
  result put(const char *path, size_t content_length,
             content_provider content_provider, const char *content_type);
  result put(const char *path, content_provider_without_length content_provider,
             const char *content_type);
  ...
  result patch(const char *path, size_t content_length,
               content_provider content_provider, const char *content_type);
  result patch(const char *path, content_provider_without_length content_provider,
               const char *content_type);
  ...
  result del(const char *path);
  result del(const char *path, const headers &headers);
  result del(const char *path, const char *body, size_t content_length,
                const char *content_type);
  result del(const char *path, const headers &headers, const char *body,
                size_t content_length, const char *content_type);
  result del(const char *path, const std::string &body,
                const char *content_type);
  result del(const char *path, const headers &headers,
                const std::string &body, const char *content_type);
  ...
  void set_socket_options(socket_options socket_options);
...
protected:
  ...
  void copy_settings(const client_impl &rhs);
  ...
  bool tcp_nodelay_ = CPPHTTPLIB_TCP_NODELAY;
  socket_options socket_options_ = nullptr;
...
private:
  ...
  std::unique_ptr<response> send_with_content_provider(
      ...
      const char *body, size_t content_length, content_provider content_provider,
      content_provider_without_length content_provider_without_length,
...
private:
  std::unique_ptr<client_impl> cli_;
...
public:
  ...
  ssl_server(X509 *cert, EVP_PKEY *private_key,
...
class ssl_client : public client_impl {
public:
  explicit ssl_client(const std::string &host);

  explicit ssl_client(const std::string &host, int port);

  explicit ssl_client(const std::string &host, int port,
                     const std::string &client_cert_path,
                     const std::string &client_key_path);

  explicit ssl_client(const std::string &host, int port, X509 *client_cert,
                     EVP_PKEY *client_key);
...
socket_t create_socket(const char *host, int port, int socket_flags,
                       bool tcp_nodelay, socket_options socket_options,
...
inline bool read_content_with_length(stream &strm, uint64_t len,
                                     progress progress,
                                     content_receiver_with_progress out) {
...
bool prepare_content_receiver(T &x, int &status,
                              content_receiver_with_progress receiver,
  ...
  if (decompress) {
    ...
    if (decompressor) {
      if (decompressor->is_valid()) {
        content_receiver_with_progress out = [&](const char *buf, size_t n,
  ...
  content_receiver_with_progress out = [&](const char *buf, size_t n, uint64_t off,
                                        uint64_t len) {
    return receiver(buf, n, off, len);
  };
  return callback(std::move(out));
...
bool read_content(stream &strm, T &x, size_t payload_max_length, int &status,
                  progress progress, content_receiver_with_progress receiver,
                  bool decompress) {
  return prepare_content_receiver(
      x, status, std::move(receiver), decompress,
      [&](const content_receiver_with_progress &out) {
        auto ret = true;
...
template <typename T>
inline bool write_content(stream &strm, const content_provider &content_provider,
...
write_content_without_length(stream &strm,
                             const content_provider &content_provider,
                             const T &is_shutting_down) {
  size_t offset = 0;
...
template <typename T, typename U>
inline bool
write_content_chunked(stream &strm, const content_provider &content_provider,
...
public:
  ...
  bool parse(const char *buf, size_t n, const content_receiver &content_callback,
             const multipart_content_header &header_callback) {
...
public:
  ...
  bool operator()(size_t offset, size_t, data_sink &sink) {
    return content_provider_(offset, sink);
  }

private:
  content_provider_without_length content_provider_;
};
...
inline multipart_form_data request::get_file_value(const char *key) const {
...
response::set_content_provider(const char *content_type,
                               content_provider_without_length provider,
                               const std::function<void()> &resource_releaser) {
  set_header("Content-Type", content_type);
  content_length_ = 0;
...
inline server::server()
    : new_task_queue(
          [] { return new thread_pool(CPPHTTPLIB_THREAD_POOL_COUNT); }),
...
inline server &server::del(const char *pattern, handler handler) {
  return del(pattern, strlen(pattern), handler);
}

inline server &server::del(const char *pattern, size_t pattern_len,
                              handler handler) {
...
inline server &server::del(const char *pattern,
                              handler_with_content_reader handler) {
  return del(pattern, strlen(pattern), handler);
}
...
inline server &server::set_error_handler(handler_with_response handler) {
...
inline server &server::set_error_handler(handler handler) {
  error_handler_ = [handler](const request &req, response &res) {
    handler(req, res);
    return handler_response::Handled;
  };
  return *this;
}

inline server &server::set_exception_handler(exception_handler handler) {
  exception_handler_ = std::move(handler);
  return *this;
}

inline server &server::set_pre_routing_handler(handler_with_response handler) {
...
inline server &server::set_socket_options(socket_options socket_options) {
...
inline bool server::write_response_core(stream &strm, bool close_connection,
  ...
  if (400 <= res.status && error_handler_ &&
      error_handler_(req, res) == handler_response::Handled) {
...
inline bool server::read_content(stream &strm, request &req, response &res) {
  multipart_form_data_map::iterator cur;
  if (read_content_core(
          ...
          // Multipart
          [&](const multipart_form_data &file) {
...
inline bool server::read_content_with_content_receiver(
    stream &strm, request &req, response &res, content_receiver receiver,
    multipart_content_header multipart_header,
    content_receiver multipart_receiver) {
...
inline bool server::listen_internal() {
  ...
  {
    std::unique_ptr<task_queue> task_queue(new_task_queue());
...
inline bool server::routing(request &req, response &res, stream &strm) {
  if (pre_routing_handler_ &&
      pre_routing_handler_(req, res) == handler_response::Handled) {
    return true;
  }
  ...
  if (detail::expect_content(req)) {
    // Content reader handler
    {
      content_reader reader(
          ...
          [&](multipart_content_header header, content_receiver receiver) {
...
inline bool server::dispatch_request_for_content_reader(
    request &req, response &res, content_reader content_reader,
    const handlers_for_content_reader &handlers) {
  for (const auto &x : handlers) {
    const auto &pattern = x.first;
    const auto &handler = x.second;
...
inline client_impl::client_impl(const std::string &host)
    : client_impl(host, 80, std::string(), std::string()) {}

inline client_impl::client_impl(const std::string &host, int port)
    : client_impl(host, port, std::string(), std::string()) {}
...
inline client_impl::~client_impl() { lock_socket_and_shutdown_and_close(); }

inline bool client_impl::is_valid() const { return true; }

inline void client_impl::copy_settings(const client_impl &rhs) {
...
inline socket_t client_impl::create_client_socket(Error &error) const {
...
inline bool client_impl::create_and_connect_socket(socket &socket,
                                                  Error &error) {
...
inline void client_impl::shutdown_ssl(socket & /*socket*/,
...
inline void client_impl::shutdown_socket(socket &socket) {
  if (socket.sock == INVALID_SOCKET) { return; }
  detail::shutdown_socket(socket.sock);
}

inline void client_impl::close_socket(socket &socket) {
...
inline void client_impl::lock_socket_and_shutdown_and_close() {
...
inline bool client_impl::read_response_line(stream &strm, const request &req,
                                           response &res) {
  std::array<char, 2048> buf;
...
inline bool client_impl::send(request &req, response &res, Error &error) {
...

  {
    ...
    if (!is_alive) {
      ...
      // TODO: refactoring
      if (is_ssl()) {
        auto &scli = static_cast<ssl_client &>(*this);
        if (!proxy_host_.empty() && proxy_port_ != -1) {
          bool success = false;
...
inline result client_impl::send(const request &req) {
  auto req2 = req;
  return send_(std::move(req2));
}

inline result client_impl::send_(request &&req) {
...
inline bool client_impl::handle_request(stream &strm, request &req,
...
inline bool client_impl::redirect(request &req, response &res, Error &error) {
  ...
  if (next_scheme == scheme && next_host == host_ && next_port == port_) {
    ...
    if (next_scheme == "https") {
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
      ssl_client cli(next_host.c_str(), next_port);
      ...
      client_impl cli(next_host.c_str(), next_port);
      cli.copy_settings(*this);
      return detail::redirect(cli, req, res, next_path, location, error);
    }
  }
...
inline bool client_impl::write_content_with_provider(stream &strm,
                                                    const request &req,
                                                    Error &error) {
...
inline bool client_impl::write_request(stream &strm, request &req,
                                      bool close_connection, Error &error) {
  // Prepare additional headers
...
inline std::unique_ptr<response> client_impl::send_with_content_provider(
    request &req,
    // const char *method, const char *path, const headers &headers,
    const char *body, size_t content_length, content_provider content_provider,
    content_provider_without_length content_provider_without_length,
...
inline result client_impl::send_with_content_provider(
    const char *method, const char *path, const headers &headers,
    const char *body, size_t content_length, content_provider content_provider,
    content_provider_without_length content_provider_without_length,
    const char *content_type) {
...
inline bool client_impl::process_request(stream &strm, request &req,
                                        response &res, bool close_connection,
                                        Error &error) {
  // Send request
  ...
  if ((res.status != 204) && req.method != "HEAD" && req.method != "CONNECT") {
    auto out =
        req.content_receiver
            ? static_cast<content_receiver_with_progress>(
...
inline bool
client_impl::process_socket(const socket &socket,
                           std::function<bool(stream &strm)> callback) {
...
inline bool client_impl::is_ssl() const { return false; }

inline result client_impl::get(const char *path) {
  return get(path, headers(), progress());
}

inline result client_impl::get(const char *path, progress progress) {
  return get(path, headers(), std::move(progress));
}

inline result client_impl::get(const char *path, const headers &headers) {
  return get(path, headers, progress());
}
...
inline result client_impl::get(const char *path, const params &params,
...
inline result client_impl::head(const char *path) {
  return head(path, headers());
}

inline result client_impl::head(const char *path, const headers &headers) {
  request req;
  req.method = "HEAD";
  req.headers = headers;
  req.path = path;
...
inline result client_impl::post(const char *path) {
  return post(path, std::string(), nullptr);
}

inline result client_impl::post(const char *path, const char *body,
                               size_t content_length,
                               const char *content_type) {
  return post(path, headers(), body, content_length, content_type);
}

inline result client_impl::post(const char *path, const headers &headers,
...
inline result client_impl::post(const char *path, const std::string &body,
                               const char *content_type) {
...
inline result client_impl::post(const char *path, const params &params) {
  return post(path, headers(), params);
}

inline result client_impl::post(const char *path, size_t content_length,
...
inline result client_impl::put(const char *path) {
  return put(path, std::string(), nullptr);
}

inline result client_impl::put(const char *path, const char *body,
                              size_t content_length, const char *content_type) {
  return put(path, headers(), body, content_length, content_type);
}

inline result client_impl::put(const char *path, const headers &headers,
                              const char *body, size_t content_length,
                              const char *content_type) {
...
inline result client_impl::put(const char *path, const std::string &body,
                              const char *content_type) {
...
inline result client_impl::put(const char *path, size_t content_length,
                              content_provider content_provider,
                              const char *content_type) {
...
inline result client_impl::put(const char *path, const params &params) {
...
inline result client_impl::patch(const char *path) {
  return patch(path, std::string(), nullptr);
}

inline result client_impl::patch(const char *path, const char *body,
                                size_t content_length,
                                const char *content_type) {
  return patch(path, headers(), body, content_length, content_type);
}

inline result client_impl::patch(const char *path, const headers &headers,
                                const char *body, size_t content_length,
                                const char *content_type) {
...
inline result client_impl::patch(const char *path, const std::string &body,
                                const char *content_type) {
...
inline result client_impl::patch(const char *path, size_t content_length,
                                content_provider content_provider,
                                const char *content_type) {
...
inline result client_impl::del(const char *path) {
  return del(path, headers(), std::string(), nullptr);
}

inline result client_impl::del(const char *path, const headers &headers) {
  return del(path, headers, std::string(), nullptr);
}

inline result client_impl::del(const char *path, const char *body,
                                 size_t content_length,
                                 const char *content_type) {
  return del(path, headers(), body, content_length, content_type);
}
...
inline result client_impl::del(const char *path, const std::string &body,
                                 const char *content_type) {
  return del(path, headers(), body.data(), body.size(), content_type);
}
...
inline result client_impl::options(const char *path) {
  return options(path, headers());
}

inline result client_impl::options(const char *path, const headers &headers) {
  request req;
  req.method = "OPTIONS";
  req.headers = headers;
  req.path = path;
...
inline size_t client_impl::is_socket_open() const {
  std::lock_guard<std::mutex> guard(socket_mutex_);
  return socket_.is_open();
}

inline void client_impl::stop() {
  std::lock_guard<std::mutex> guard(socket_mutex_);
...
inline void client_impl::set_connection_timeout(time_t sec, time_t usec) {
  connection_timeout_sec_ = sec;
  connection_timeout_usec_ = usec;
}

inline void client_impl::set_read_timeout(time_t sec, time_t usec) {
  read_timeout_sec_ = sec;
  read_timeout_usec_ = usec;
}

inline void client_impl::set_write_timeout(time_t sec, time_t usec) {
  write_timeout_sec_ = sec;
  write_timeout_usec_ = usec;
}

inline void client_impl::set_basic_auth(const char *username,
                                       const char *password) {
  basic_auth_username_ = username;
  basic_auth_password_ = password;
}

inline void client_impl::set_bearer_token_auth(const char *token) {
  bearer_token_auth_token_ = token;
}

#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
inline void client_impl::set_digest_auth(const char *username,
                                        const char *password) {
...
inline void client_impl::set_keep_alive(bool on) { keep_alive_ = on; }

inline void client_impl::set_follow_location(bool on) { follow_location_ = on; }

inline void client_impl::set_default_headers(headers headers) {
  default_headers_ = std::move(headers);
}

inline void client_impl::set_tcp_nodelay(bool on) { tcp_nodelay_ = on; }

inline void client_impl::set_socket_options(socket_options socket_options) {
  socket_options_ = std::move(socket_options);
}

inline void client_impl::set_compress(bool on) { compress_ = on; }

inline void client_impl::set_decompress(bool on) { decompress_ = on; }

inline void client_impl::set_interface(const char *intf) { interface_ = intf; }

inline void client_impl::set_proxy(const char *host, int port) {
  proxy_host_ = host;
  proxy_port_ = port;
}

inline void client_impl::set_proxy_basic_auth(const char *username,
                                             const char *password) {
  proxy_basic_auth_username_ = username;
  proxy_basic_auth_password_ = password;
}

inline void client_impl::set_proxy_bearer_token_auth(const char *token) {
  proxy_bearer_token_auth_token_ = token;
}

#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
inline void client_impl::set_proxy_digest_auth(const char *username,
                                              const char *password) {
...
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
inline void client_impl::enable_server_certificate_verification(bool enabled) {
  server_certificate_verification_ = enabled;
}
#endif

inline void client_impl::set_logger(logger logger) {
  logger_ = std::move(logger);
}
...
inline ssl_server::ssl_server(X509 *cert, EVP_PKEY *private_key,
...
inline bool ssl_server::is_valid() const { return ctx_; }

inline bool ssl_server::process_and_close_socket(socket_t sock) {
...
inline ssl_client::ssl_client(const std::string &host)
    : ssl_client(host, 443, std::string(), std::string()) {}

inline ssl_client::ssl_client(const std::string &host, int port)
    : ssl_client(host, port, std::string(), std::string()) {}

inline ssl_client::ssl_client(const std::string &host, int port,
                            const std::string &client_cert_path,
                            const std::string &client_key_path)
    : client_impl(host, port, client_cert_path, client_key_path) {
...
inline ssl_client::~ssl_client() {
  ...
  ssl_client::shutdown_ssl(socket_, true);
}

inline bool ssl_client::is_valid() const { return ctx_; }

inline void ssl_client::set_ca_cert_path(const char *ca_cert_file_path,
                                        const char *ca_cert_dir_path) {
  if (ca_cert_file_path) { ca_cert_file_path_ = ca_cert_file_path; }
  if (ca_cert_dir_path) { ca_cert_dir_path_ = ca_cert_dir_path; }
}

inline void ssl_client::set_ca_cert_store(X509_STORE *ca_cert_store) {
  if (ca_cert_store) {
    if (ctx_) {
...
inline long ssl_client::get_openssl_verify_result() const {
  return verify_result_;
}

inline SSL_CTX *ssl_client::ssl_context() const { return ctx_; }

inline bool ssl_client::create_and_connect_socket(socket &socket, Error &error) {
  return is_valid() && client_impl::create_and_connect_socket(socket, error);
}

// Assumes that socket_mutex_ is locked and that there are no requests in flight
inline bool ssl_client::connect_with_proxy(socket &socket, response &res,
...
inline bool ssl_client::load_certs() {
  bool ret = true;
...
inline bool ssl_client::initialize_ssl(socket &socket, Error &error) {
  auto ssl = detail::ssl_new(
...
inline void ssl_client::shutdown_ssl(socket &socket, bool shutdown_gracefully) {
...
inline bool
ssl_client::process_socket(const socket &socket,
                          std::function<bool(stream &strm)> callback) {
  assert(socket.ssl);
...
inline bool ssl_client::is_ssl() const { return true; }

inline bool ssl_client::verify_host(X509 *server_cert) const {
...
inline bool
ssl_client::verify_host_with_subject_alt_name(X509 *server_cert) const {
  auto ret = false;
...
inline bool ssl_client::verify_host_with_common_name(X509 *server_cert) const {
...
inline bool ssl_client::check_host_name(const char *pattern,
...
inline client::client(const char *scheme_host_port,
  ...
  if (std::regex_match(scheme_host_port, m, re)) {
    ...
    if (is_ssl) {
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
      cli_ = detail::make_unique<ssl_client>(host.c_str(), port,
                                            client_cert_path, client_key_path);
      is_ssl_ = is_ssl;
#endif
    } else {
      cli_ = detail::make_unique<client_impl>(host.c_str(), port,
                                             client_cert_path, client_key_path);
    }
  } else {
    cli_ = detail::make_unique<client_impl>(scheme_host_port, 80,
                                           client_cert_path, client_key_path);
  }
}

inline client::client(const std::string &host, int port)
    : cli_(detail::make_unique<client_impl>(host, port)) {}

inline client::client(const std::string &host, int port,
                      const std::string &client_cert_path,
                      const std::string &client_key_path)
    : cli_(detail::make_unique<client_impl>(host, port, client_cert_path,
                                           client_key_path)) {}
...
inline result client::get(const char *path, content_receiver content_receiver) {
...
inline result client::get(const char *path, response_handler response_handler,
                          content_receiver content_receiver) {
...
inline result client::del(const char *path) { return cli_->del(path); }
inline result client::del(const char *path, const headers &headers) {
  return cli_->del(path, headers);
}
inline result client::del(const char *path, const char *body,
                             size_t content_length, const char *content_type) {
  return cli_->del(path, body, content_length, content_type);
}
inline result client::del(const char *path, const headers &headers,
                             const char *body, size_t content_length,
                             const char *content_type) {
  return cli_->del(path, headers, body, content_length, content_type);
}
inline result client::del(const char *path, const std::string &body,
                             const char *content_type) {
  return cli_->del(path, body, content_type);
}
inline result client::del(const char *path, const headers &headers,
                             const std::string &body,
                             const char *content_type) {
  return cli_->del(path, headers, body, content_type);
}
...
inline void client::set_socket_options(socket_options socket_options) {
...
inline void client::set_ca_cert_path(const char *ca_cert_file_path,
                                     const char *ca_cert_dir_path) {
  if (is_ssl_) {
    static_cast<ssl_client &>(*cli_).set_ca_cert_path(ca_cert_file_path,
                                                     ca_cert_dir_path);
  }
}
...
inline void client::set_ca_cert_store(X509_STORE *ca_cert_store) {
  if (is_ssl_) {
    static_cast<ssl_client &>(*cli_).set_ca_cert_store(ca_cert_store);
  }
}

inline long client::get_openssl_verify_result() const {
  if (is_ssl_) {
    return static_cast<ssl_client &>(*cli_).get_openssl_verify_result();
  }
  return -1; // NOTE: -1 doesn't match any of X509_V_ERR_???
}

inline SSL_CTX *client::ssl_context() const {
  if (is_ssl_) { return static_cast<ssl_client &>(*cli_).ssl_context(); }
  return nullptr;
}
#endif
