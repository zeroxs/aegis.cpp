...
namespace aegis
{
    ...
    public:
         ...
         */
        std::shared_ptr<rest::rest_controller> get_rest_controller() noexcept { return _rest; }

        /// Get the ratelimit object
        /**
         * @returns Reference to the internal rate limiter
         */
        std::shared_ptr<ratelimit_mgr_t> get_ratelimit() noexcept { return _ratelimit; }

        /// Get the shard manager
        /**
         * @returns Reference to the internal shard manager
         */
        std::shared_ptr<shards::shard_mgr> get_shard_mgr() noexcept { return _shard_mgr; }
...
#pragma region event handlers
        ...
        /// FULL RAW EVENT callback (only raw)
        void set_manage_myself_any_event(raw_event_t cb) { i_any_raw = cb; }
...
#pragma region event handlers
        ...
        AEGIS_DECL void ws_lsw_handle_any(const json& result, shards::shard* _shard);
#if !defined(AEGIS_DISABLE_ALL_CACHE)
...
#pragma endregion
        ...
        std::chrono::hours _tz_bias = std::chrono::hours(0);
    public:
