...
namespace aegis
{
    ...
    {
        _shard_mgr = std::make_shared<shards::shard_mgr>(_token, *_io_context, log, _cluster_id, _max_cluster_id);
    ...
    AEGIS_DECL LSW::v5::Tools::Future<gateway::objects::message> core::create_dm_message(snowflake member_id, const std::string& content, int64_t nonce)
    {
        ...
        if (!m)
            throw std::exception("Member error");//return aegis::make_exception_future<gateway::objects::message>(aegis::error::member_error);
        if (m->get_dm_id())
        ...
        else
        {
            ...
            auto res = get_ratelimit()->post_task<gateway::objects::channel>(params)
                .then<gateway::objects::message>([=](gateway::objects::channel&& reply) {
        ...
        try
        {
            ...
            if (!result.is_null())
            {
                ...
                if (!result["t"].is_null())
                {
                    ...
                    asio::post(*_io_context, [=, dothat = (it != ws_handlers.end()), res = std::move(result)]()
                    {
                        ...
                        try
                        {
                            ...
                            if (message_end)
                                message_end(s_t, cmd);
#else
                            ws_lsw_handle_any(res, _shard); // lsw
                            if (dothat) (it->second)(res, _shard);
#endif
                        }
                            catch (std::exception& e)
                            {
    ...
    AEGIS_DECL void core::ws_lsw_handle_any(const json& result, shards::shard* _shard)
    {
        if (i_any_raw)
            i_any_raw(result, _shard);
    }
    ...
    )
    {
        ...
        ///auto res = get_ratelimit()->post_task({ "/guilds", rest::Post, obj.dump() }).get();
        ///return LSW::v5::Tools::Future<gateway::objects::guild>([=] {return gateway::objects::guild(json::parse(res.content)); });
        //return get_ratelimit()->post_task<gateway::objects::guild>({ fmt::format("/channels/{}/messages", channel_id), rest::Post, obj.dump() });
    ...
    AEGIS_DECL LSW::v5::Tools::Future<gateway::objects::message> core::create_message(snowflake channel_id, const std::string& msg, int64_t nonce, bool perform_lookup)
    {
        ...
        if (perform_lookup)
        {
            auto it = channels.find(channel_id);
            if (it == channels.end())
                throw std::exception("Channel not found");//return aegis::make_exception_future<gateway::objects::message>(error::channel_not_found);
