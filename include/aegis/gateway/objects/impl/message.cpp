    ...
    namespace gateway
    {
        ...
        namespace objects
        {
            ...
            AEGIS_DECL aegis::guild& message::get_guild()
            {
                ...
                if (_guild == nullptr)
                    throw std::exception("message::get_guild() _guild = nullptr");//throw exception("message::get_guild() _guild = nullptr", make_error_code(error::guild_not_found));
                return *_guild;
            }
            ...
            AEGIS_DECL aegis::channel& message::get_channel()
            {
                ...
                if (_channel == nullptr)
                    throw std::exception("message::get_channel() _channel == nullptr");//throw exception("message::get_channel() _channel == nullptr", make_error_code(error::channel_not_found));
                return *_channel;
            }
            ...
            AEGIS_DECL aegis::user& message::get_user()
            {
                ...
                {
                    if (_author_id == 0)
                        throw std::exception("message::get_member() _member == nullptr && _author_id == 0");//throw exception("message::get_member() _member == nullptr && _author_id == 0", make_error_code(error::member_not_found));
            ...
            AEGIS_DECL LSW::v5::Tools::Future<rest::rest_reply> message::delete_message()
            {
                populate_self();
                assert(_channel != nullptr);
                if (_channel == nullptr)
                    throw std::exception("Channel error");//return aegis::make_exception_future<rest::rest_reply>(std::make_exception_ptr(aegis::exception(make_error_code(error::channel_error))));

                return get_channel().delete_message(_message_id);
            }

            AEGIS_DECL LSW::v5::Tools::Future<message> message::edit(const std::string& content)
            {
                populate_self();
                assert(_channel != nullptr);
                if (_channel == nullptr)
                    throw std::exception("Channel error");//return aegis::make_exception_future<message>(std::make_exception_ptr(aegis::exception(make_error_code(error::channel_error))));

                return get_channel().edit_message(_message_id, content);
            }

            AEGIS_DECL LSW::v5::Tools::Future<message> message::edit(edit_message_t& obj)
            {
                populate_self();
                assert(_channel != nullptr);
                if (_channel == nullptr)
            ...
            AEGIS_DECL void message::populate_self()
            {
                ...
                if (!_channel)
                    ...
                    throw std::exception("Somehow channel not found");//throw aegis::exception(error::channel_not_found);
                if (_guild == nullptr)
                ...
                if (!is_webhook())
                {
                    ...
                    if (_user == nullptr)
                    {
                        ...
                        //throw std::exception("Somehow user not found");
